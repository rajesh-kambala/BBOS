...
//
// Filename: bbos.js (Spanish Version with Translations)
// Description:	
//
// Notes:		
//
...
function findString(str) {
    ...
    if (window.find) {
        ...
        bbAlert("Los navegadores Opera no son compatibles, lo sentimos...")
        return;
    }
    if (!strFound) bbAlert("¡No se encontró la cadena '" + str + "'!")
    return;
}
...
function displayMap() {
    ...
    if (checkedCount = 0) {
        bbAlert("Seleccione las compañías que quiere mostrar en el mapa.");
        return;
    }

    if (checkedCount > 50) {
        bbAlert("Solo 50 compañías se pueden mostrar en el mapa al mismo tiempo.");
        return;
    }
...
function displayUserListMap() {
    ...
    if (checkedCount = 0) {
        bbAlert("Seleccione las Categorías de Vigilancia que quiere mostrar en el mapa.");
        return;
    }
...
function ValidateExportsCount2(hidExportsMax, hidExportsUsed, hidSelectedCount, hidExportsPeriod) {
    ...
    var Period = "d&iacute;a";
    if (iExportsPeriod == "M") {
        Period = "mes"
    }

    if (iExportsMax == 0) {
        bbAlert("Su membres&iacute;a no permite ninguna exportaci&oacute;n.", "Se ha excedido el n&uacute;mero de exportaciones");
        return false;
    }
    else if ((iExportsUsed + iSelectedCount) > iExportsMax) {
        var msg = "Su membres&iacute;a s&oacute;lo permite un m&aacute;ximo de " + iExportsMax + " exportaciones por " + Period;

        if (iExportsUsed > 0)
            msg += ", y ya ha exportado " + iExportsUsed + ".";
        else
            msg += ".";

        msg += "  Seleccione menos art&iacute;culos para esta exportaci&oacute;n.";

        bbAlert(msg, "Se ha excedido el n&uacute;mero de exportaciones");
        return false;
    }
