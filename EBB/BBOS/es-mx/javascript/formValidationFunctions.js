//***********************************************************************
//***********************************************************************
// Copyright Travant Solutions, Inc. 2000-2023
//
...
//
// Filename:		formValidationFunctions.js (Spanish Version w/ Translations)
// Description:	
//
// Notes:		
//
...
function validateForm(form)
{
        ...
        if ((e.type == "select-one") || (e.type == "select-multiple"))  {
            ...
            if ((e.value.length > 0) && ((e.getAttribute('tsiInteger') != null) || (e.getAttribute('tsiNumeric') != null) || (e.getAttribute('tsiMin') != null) || (e.getAttribute('tsiMax') != null))) {
                ...
                if (!bIsValid) {
                    ...
                    szNumericErrors += " - El campo " + e.getAttribute('tsiDisplayName') + " debe ser un n&uacute;mero"
                    szNumericErrors += ".\n";
                } else {
                        ...
                        ((e.getAttribute('tsiMax') != null) && (nValue > e.getAttribute('tsiMax'))))
                    {
                        ...
                        if (e.getAttribute('tsiMin') != null)
                            szNumericErrors += " que sea mayor que o igual a " + e.getAttribute('tsiMin');

                        if ((e.getAttribute('tsiMax') != null) && (e.getAttribute('tsiMin') != null))
                            szNumericErrors += " y menos que o igual a " + e.getAttribute('tsiMax');
                        else if (e.getAttribute('tsiMax') != null)
                            szNumericErrors += " que es menos que o igual a " + e.getAttribute('tsiMax');
                ...
                if ((e.getAttribute('tsiEquals') != null) && (nValue != e.getAttribute('tsiEquals'))) {
                  	if (!szRequiredErrors && !szNumericErrors && !szDateErrors)
                        ...
                        szNumericErrors += " - El campo " + e.getAttribute('tsiDisplayName');
				    if (e.getAttribute('tsiEquals') != null)
                        szNumericErrors += " debe ser igual a " + e.getAttribute('tsiEquals');
            ...
            if ((e.value.length > 0) && e.getAttribute('tsiCurrency')) {
                ...
                if (!bIsValid) {
                    if (!szRequiredErrors && !szNumericErrors && !szDateErrors)
                        eToFocus = e;
                    szNumericErrors += " - El campo " + e.getAttribute('tsiDisplayName') + " debe tener un monto de moneda valido.\n";
                }
				...
				if (bContinue) {
	                if (!IsValidDate(e.value, b4DigitYear)) {
	                    ...
	                    if (e.getAttribute('tsiDisplayName') == null)
                            szDateErrors += " - El campo Fecha debe ser una fecha valida en el formato " + szDateFormat + ".\n";
	                    else
                            szDateErrors += " - El campo " + e.getAttribute('tsiDisplayName') + " debe ser una fecha valida en el formato " + szDateFormat + ".\n";
            ...
            if ((e.value.length > 0) && e.getAttribute('tsiDateTime')) {
				...
				if (bContinue) {
	                if (!IsValidDateTime(e.value)) {
	                    ...
	                    if (e.getAttribute('tsiDisplayName') == null)
                            szDateErrors += " - El campo Fecha/Hora debe ser una fecha valida en el formato " + szDateFormat + ".\n";
	                    else
                            szDateErrors += " - El campo " + e.getAttribute('tsiDisplayName') + " debe ser una fecha/hora valida en el formato " + szDateFormat + " hh:mm am.\n";
	                } // End if (!IsValidDate(e.value, b4DigitYear))
            ...
            if ((e.value.length > 0) && e.getAttribute('tsiEmail')) {
				if (!isValidEMail(e.value)) {
                    szGeneralErrors += " - El campo " + e.getAttribute('tsiDisplayName') + " debe ser una direcci&oacute;n de correo electr&oacute;nico de Internet valida.\n";
				}
            ...
            if ((e.value.length > 0) && e.getAttribute('tsiPhone')) {
				if (!isValidPhone(e.value)) {
                    szGeneralErrors += " - El campo " + e.getAttribute('tsiDisplayName') + " debe ser un n&uacute;mero de teléfono valido.\n";
				}
			} // End If e.getAttribute('tsiPhone')				

            if ((e.value.length > 0) && e.getAttribute('tsiZip')) {
				if (!isValidZip(e.value)) {
                    szGeneralErrors += " - El campo " + e.getAttribute('tsiDisplayName') + " debe ser un c&oacute;digo postal de EE. UU. o Canada valido.\n";
				}
			...
			if ((e.value.length > 0) && e.getAttribute('tsiMaxLength')) {
				if (e.value.length > e.getAttribute('tsiMaxLength')) {
                    szGeneralErrors += " - The value for the field " + e.getAttribute('tsiDisplayName') + " excede la longitud maxima permitida de " + e.getAttribute('tsiMaxLength') + " caracteres.\n";								
				}
			}
    ...
    if (szRequiredErrors || szNumericErrors || szDateErrors || szGeneralErrors) {
		...
		szErrorMessage = "";
        if (szRequiredErrors) {
            szErrorMessage += " - Los siguientes campos obligatorios estan vacios:\n";

			if (bUnknownAcceptable) {			
                szErrorMessage += "   (Algunos campos aceptan 'N/A' o 'No se conoce')\n";
    		}        
...
function IsValidDate(szText, b4DigitYear) {
    ...
    //Spanish format
    szDay = szDate.substring(0, iSlash1);
    szMonth = szDate.substring(iSlash1+1,iSlash2);
...
function displayErrorMessage(szMsg) {
    szMsg = szMsg.replaceAll("\n\n", "<br />");
    szMsg = szMsg.replaceAll("\n", "<br />");
    bootbox.alert({
        title: "Corrija los siguientes errores", message: szMsg });
}
...
function IsValidDateRange(eFromDate, eToDate) {
	...
	if ((fromValue.length > 0) &&
		...
		if (dtFromDate > dtToDate) {
            return "Un rango de fechas no valido se ha especificado para los campos " + eFromDate.getAttribute('tsiDisplayName') + " y " + eToDate.getAttribute('tsiDisplayName') + " fields.\n";
		}
	}
...
function IsValidDateRange2(eFromDate, eToDate) {
	...
	if ((fromValue.length > 0) &&
		(toValue.length == 0)) {
        return "Cuando se especifica un valor para el campo " + eFromDate.getAttribute('tsiDisplayName') + " field, se necesita un valor para el campo " + eToDate.getAttribute('tsiDisplayName') + " field.";
	}

	if ((toValue.length > 0) &&
		(fromValue.length == 0)) {
        return "Cuando se especifica un valor para el campo " + eToDate.getAttribute('tsiDisplayName') + " field, se necesita un valor para el campo " + eFromDate.getAttribute('tsiDisplayName') + " field.";
	}
...
function IsValidDateRangeMsg2(eFromDate, eToDate) {
	...
	if ((fromValue.length > 0) &&
		...
		if (dtFromDate > dtToDate) {
            return "- El campo " + eFromDate.getAttribute('tsiDisplayName') + " debe ser antes que el campo " + eToDate.getAttribute('tsiDisplayName') + ".\n";
		}
	}
...
function IsValidNumericRange(eFrom, eTo) {
	...
	if (fromValue > toValue) {
        return "Se ha especificado un rango no valido para el campo " + eFrom.getAttribute('tsiDisplayName') + " y " + eTo.getAttribute('tsiDisplayName') + " fields.\n";
	}
...
// The passed date should be a valid date in the format ("dd/mm/yyyy")
//
