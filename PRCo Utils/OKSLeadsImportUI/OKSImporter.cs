...
namespace OKSLeadsImportUI
{
    public class OKSImporter
    {
        ...
        public int CompanyInsertedCount = 0;
        public int AddresInsertedCount = 0;
        public int EmailInsertedCount = 0;
        public int WebInsertedCount = 0;
        public int PhoneInsertedCount = 0;
        public int FaxInsertedCount = 0;
        public int ClassificationInsertedCount = 0;
        public int SpecieInsertedCount = 0;
        public int PersonInsertedCount = 0;
        public int CompanyProfileInsertedCount = 0;
        ...
        public void ExecuteImport(object form)
        {
            ...
            try
            {
                ...
                _classifConnection = new OleDbConnection(msExcelConn);

                _dbConn = new SqlConnection(ConfigurationManager.ConnectionStrings[Destination + "CRM"].ConnectionString);
                _dbLumberLeadsConn = new SqlConnection(ConfigurationManager.ConnectionStrings[Destination + "LumberLeads"].ConnectionString);
                ...
                OleDbCommand cmdProfileCount = driverConnection.CreateCommand();
                cmdProfileCount.CommandText = "SELECT COUNT(1) FROM [Output$]";
                int count = (int)cmdProfileCount.ExecuteScalar();


                form1.initProgressBar(count);

                OleDbCommand cmdProfileOutput = driverConnection.CreateCommand();
                cmdProfileOutput.CommandText = "SELECT * FROM [Output$] ORDER BY LEAD_ID";
                    ...
                    while (drProfileOuput.Read())
                    {
                        ...
                        if (bInsertCompany)
                        {
                            ...
                            try
                            {
                                ...
                                cmdProfilePersonnel.CommandText = "SELECT * FROM [Personnel$] WHERE LEAD_ID=" + iLeadID.ToString();
            ...
            finally
            {
                ...
                WriteLine("        Companies Skipped: " + SkippedCount.ToString("###,##0"));
                WriteLine("       Companies Inserted: " + CompanyInsertedCount.ToString("###,##0"));
                WriteLine("  CompanProfiles Inserted: " + CompanyProfileInsertedCount.ToString("###,##0"));
                WriteLine("       Addresses Inserted: " + AddresInsertedCount.ToString("###,##0"));
                WriteLine("          Emails Inserted: " + EmailInsertedCount.ToString("###,##0"));
                WriteLine("        Websites Inserted: " + WebInsertedCount.ToString("###,##0"));
                WriteLine("          Phones Inserted: " + PhoneInsertedCount.ToString("###,##0"));
                WriteLine("           Faxes Inserted: " + FaxInsertedCount.ToString("###,##0"));
                WriteLine(" Classifications Inserted: " + ClassificationInsertedCount.ToString("###,##0"));
                WriteLine("         Species Inserted: " + SpecieInsertedCount.ToString("###,##0"));
                WriteLine("         Persons Inserted: " + PersonInsertedCount.ToString("###,##0"));
                ...
                if (_lszSkippedBuffer.Count > 0) {
                    szOutputFile = Path.Combine(OutputFolder, filePrefix + " Skipped.txt");
                    using (StreamWriter sw = new StreamWriter(szOutputFile))
                    {
                ...
                {
                    szOutputFile = Path.Combine(OutputFolder, filePrefix + " UnknownCity.txt");
                    using (StreamWriter sw = new StreamWriter(szOutputFile))
                    {
        ...
        private const string SQL_PHONE_UPDATE = "UPDATE Phone SET Phon_AreaCode=@AreaCode, Phon_Number=@Number, phon_PRPublish=@PRPublish, phon_PRPreferredPublished=@PRPublish, phon_PRPreferredInternal=@Default, Phon_UpdatedBy=@UpdatedBy, Phon_UpdatedDate=@UpdatedDate WHERE Phon_PhoneId = @PhoneID";
        private const string SQL_PHONE_INSERT = "INSERT INTO Phone (phon_PhoneID, phon_CompanyID, phon_Type, phon_PRDescription, Phon_AreaCode, Phon_Number, Phon_CountryCode, phon_PRPublish, phon_PRPreferredPublished, phon_PRPreferredInternal, Phon_CreatedBy, Phon_CreatedDate, Phon_UpdatedBy, Phon_UpdatedDate, Phon_Timestamp) " +
                                                           "VALUES (@PhoneID, @CompanyID, @Type, @Description, @AreaCode, @Number, @CountryCode, @PRPublish, @PRPublish, @Default, @UpdatedBy, @UpdatedDate, @UpdatedBy, @UpdatedDate, @Timestamp)";
        ...
        private const string SQL_INTERNET_UPDATE = "UPDATE Email SET Emai_EmailAddress=@Email, emai_PRWebAddress=@WebAddress, emai_PRPublish=@PRPublish, emai_PRPreferredPublished=@PRPublish, emai_PRPreferredInternal=@Default, emai_UpdatedBy=@UpdatedBy, emai_UpdatedDate=@UpdatedDate WHERE Emai_EmailId = @InternetID";
        private const string SQL_INTERNET_INSERT = "INSERT INTO Email (Emai_EmailId, Emai_CompanyID, Emai_Type, Emai_PRDescription, Emai_EmailAddress, emai_PRWebAddress, Emai_PRPublish, emai_PRPreferredPublished, emai_PRPreferredInternal, Emai_CreatedBy, Emai_CreatedDate, Emai_UpdatedBy, Emai_UpdatedDate, Emai_Timestamp) " +
                                                   "VALUES (@InternetID, @CompanyID, @Type, @Description, @Email, @WebAddress, @PRPublish, @PRPublish, @Default, @UpdatedBy, @UpdatedDate, @UpdatedBy, @UpdatedDate, @Timestamp)";
        ...
        protected void AddClassifications(int iLeadID, int iCompanyID, SqlTransaction oTran)
        {
            ...
            cmdClassification.CommandText = "SELECT OKS_CATEGORY FROM [Classifications$] WHERE LEAD_ID = " + iLeadID.ToString();
        ...
        protected int GetClassificationCount(int iLeadID)
        {
            ...
            cmdClassificationCount.CommandText = "SELECT COUNT(1) As CategoryCount FROM [Classifications$] WHERE LEAD_ID = " + iLeadID.ToString();
