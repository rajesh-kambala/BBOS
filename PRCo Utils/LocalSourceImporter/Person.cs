...
namespace LocalSourceImporter
{
    public class Person : MeisterMediaBase
    {
        ...
        public const string COL_MMWID = "Customer ID";
        ...
        public const string COL_FUNCTION_TITLE = "Function";
        ...
        {
            MMWID = csv[COL_MMWID];
            //Email = GetEligibleEmail(csv[COL_EMAIL]);

            FirstName = TranslateSpanishCharacters(prepareString(csv[COL_FIRSTNAME]));
            LastName = TranslateSpanishCharacters(prepareString(csv[COL_LASTNAME]));
            AddPhone(csv, "C");

            Title = TranslateSpanishCharacters(prepareString(csv[COL_TITLE]));
            ProcessTitle();
        ...
        protected const string SQL_PERSON_INSERT =
             "INSERT INTO Person (Pers_PersonId, pers_PRMMWID, Pers_FirstName, Pers_LastName, pers_PRLocalSource, Pers_CreatedBy, Pers_CreatedDate, Pers_UpdatedBy, Pers_UpdatedDate, Pers_Timestamp) " +
                          "VALUES (@Pers_PersonId, @pers_PRMMWID, @Pers_FirstName,@Pers_LastName, @pers_PRLocalSource, @CreatedBy, @CreatedDate, @UpdatedBy, @UpdatedDate, @Timestamp)";
        ...
        public void Save(SqlTransaction sqlTrans, bool newCompany)
        {
            ...
            if (GenericTitle.Length > 40)
            {
                GenericTitle = GenericTitle.Substring(0, 40);
            }
            ...
            if (transactionID == 0)
            {
                StringBuilder sbExplanation = new StringBuilder("Person created from Local Source import." + Environment.NewLine);
                sbExplanation.Append("MMW ID: " + MMWID + Environment.NewLine);
                transactionID = OpenPIKSTransaction(sqlTrans, 0, PersonID, sbExplanation.ToString());
            }
            ...
            // Keyword: LSS2019
            // Keep cell phones for now.
            //Phone.DeleteAll(sqlTrans, 0, PersonID);
            //if (CellPhone != null)
            //{
            //    CellPhone.PersonID = PersonID;
            //    CellPhone.CompanyID = CompanyID;
            //    CellPhone.Save(sqlTrans, true);
            //}
            ...
            UnpublishEmail(sqlTrans, 0, PersonID);
        ...
        {
            if (!csv.HasHeader(Phone.COL_MOBILE))
                return;

            if ((string.IsNullOrEmpty(csv[Phone.COL_MOBILE])) ||
               (csv[Phone.COL_MOBILE].Length < 10))
            {
                return;
            }
            ...
            CellPhone = new Phone();
            CellPhone.Load(csv, type);
            CellPhone.CompanyID = CompanyID;
            CellPhone.PersonID = PersonID;
            CellPhone.Sequence = 1;
        }

        private const string SQL_MATCH_MMWID =
               @"SELECT DISTINCT TOP 1 pers_PersonID, peli_PRStatus
        ...
        private void ProcessJobFunction(string values)
        {
            ...
            switch (value)
            {
                ...
                case "SM":
                    ...
                    return;
                case "OWN": 
                    if (string.IsNullOrEmpty(Title)) Title = "Owner";
                    if (string.IsNullOrEmpty(GenericTitle)) GenericTitle = "OWN"; 
                    Role = ",E,HE,";
                    return;
                case "SR": 
                    if (string.IsNullOrEmpty(Title)) Title = "Sales";
                    if (string.IsNullOrEmpty(GenericTitle)) GenericTitle = "SALE";
                    Role = ",S,"; 
                    return;
                case "PA": 
                case "P":
                    if (string.IsNullOrEmpty(Title)) Title = "Buyer";
                    if (string.IsNullOrEmpty(GenericTitle)) GenericTitle = "BUYR";
                    Role = ",B,"; 
                    return;
