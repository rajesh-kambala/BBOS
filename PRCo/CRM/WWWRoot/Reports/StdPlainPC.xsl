...
<HTML>
  <HEAD>
    ...
    <LINK REL="stylesheet" HREF="../reports/stdplain.css"/>
    <TITLE>CRM Report</TITLE>
  </HEAD>
  <BODY ONLOAD="AdjustImgPaths();">
...
<xsl:template match="COLUMNHEADER">
  <TR>
    <xsl:for-each select="COLUMNHEADERFIELD">
      <TD CLASS="REPORTCOLUMNHEADER" NOWRAP="TRUE">
...
<xsl:template match="DETAIL">
  <TR>
    <xsl:attribute name="STYLE">background:
      <xsl:choose>
        <xsl:when test="@ROW[.='E']">#DEDEE7</xsl:when>
        <xsl:when test="@ROW[.='O']">#F7F7F7</xsl:when>
      </xsl:choose>
    </xsl:attribute>
...
<xsl:template match="GROUPFOOTER">
  <TR>
    <xsl:for-each select="GROUPFOOTERFIELD">
      <TD CLASS="REPORTGROUPFOOTER" ALIGN="RIGHT">
        <xsl:value-of select="."/>
      </TD>
    </xsl:for-each>
  </TR>
...
<xsl:template match="SUMMARY">
  <TR>
    <xsl:for-each select="SUMMARYFIELD">
      <TD CLASS="REPORTSUMMARY" ALIGN="RIGHT">
        <xsl:value-of select="."/>
      </TD>
    </xsl:for-each>
  </TR>
