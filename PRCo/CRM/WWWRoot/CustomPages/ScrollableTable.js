...
function makeScrollableTable(tbl,scrollFooter,height){
    ...
    c = container.length;
    container[c] = document.createElement("span");
    container[c].setAttribute("style", "height: 100px; overflow: auto;");
    ...
    hdr.id += 'Header';
    hdr.style.width = tbl.clientWidth + 2 * tbl.clientLeft;// + scrollbarWidth();
    hdr.appendChild(tbl.tHead.cloneNode(true));
    tbl.tHead.style.display = 'none';

    if (!scrollFooter || !tbl.tFoot){
        ftr = document.createElement('span');
        ftr.setAttribute("style", "width:1;height:1;clip: rect(0 1 1 0);background-color:transparent;");
