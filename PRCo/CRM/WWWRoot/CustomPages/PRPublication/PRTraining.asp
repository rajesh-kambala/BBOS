...
function doPage()
{
    ...
    blkContent.Contents += "<script type=\"text/javascript\" src=\"PRTraining.js\"></script>";
    ...
    var blkEntry = eWare.GetBlock("PRPATraining");
    blkEntry.Title = "BBOS Training Article";
    blkEntry.GetEntry("prpbar_CategoryCode").LookUpFamily = "prpbar_CategoryCodeTraining";
    ...
    var sListingAction = eWare.URL("PRPublication/PRTrainingArticleListing.asp");
    ...
    if ( eWare.Mode == Save ) {
        if (blkEntry.Validate()) {
            ...
            recTrainingArticle.prpbar_PublicationCode = "TRN";
            if (prpbar_publicationarticleid == -1)
                recTrainingArticle.prpbar_Sequence = 999;
            ...
            recTrainingArticle.prpbar_IndustryTypeCode = sPublicationCode;
    ...
    if ( eWare.Mode == Edit ) {
        ...
        blkEntry.GetEntry("prpbar_FileName").Caption = "Video / Web Page URL";
        ...
        if (prpbar_publicationarticleid == -1) {
            ...
            if (recTrainingArticle.prpbar_IndustryTypeCode.indexOf(',P,T,S,') > -1) {
                sBBOSChecked = " checked ";
            }

            if (recTrainingArticle.prpbar_IndustryTypeCode.indexOf(',L,') > -1) {
...
function sSummaryAction(PublicationArticleId)
{
    if (String(PublicationArticleId).length > 0)
        return eWare.URL("PRPublication/PRTraining.asp") + "&prpbar_PublicationArticleID=" + String(PublicationArticleId);
    else
        return eWare.URL("PRPublication/PRTraining.asp");
}
%>
