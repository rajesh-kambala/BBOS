...
namespace PRCo.BBS.BBSMonitor {
    ...
    /// </summary>
    public class AUSReportEvent : AUSReportEventBase {
        ...
        string _szAttachmentName;
        string _szSubject;
        string _szSubject_Spanish;
        string _szCategory;
        ...
        override public void Initialize(int iIndex) {
            ...
            try {
                ...
                _lEventInterval = Convert.ToInt64(Utilities.GetIntConfigValue("AUSInterval", 1440)) * ONE_MINUTE_IN_MILLISECONDS;
                _oLogger.LogMessage("AUS Interval: " + _lEventInterval.ToString());

                _dtStartDateTime = Convert.ToDateTime(Utilities.GetConfigValue("AUSStartDateTime"));
                ...
                LogEventError("Error initializing AUSREport event.", e);
                throw;
            }
        }
        ...
        override public void ProcessEvent() {
            ...
            _iMonitoredCompanyID = Utilities.GetIntConfigValue("AUSMonitoredCompanyID", 0);
            ...
            try {
                ...
                _dtStartDate = GetDateTimeCustomCaption(_oConn, "prau_LastRunDate");
                ...
                _iEmailCount = 0;
                _iFaxCount = 0;
                _iMaxErrorCount = Utilities.GetIntConfigValue("AUSMaxErrorCount", 0);
                _iMaxReportCount = Utilities.GetIntConfigValue("AUSMaxReportCount", 999999999);
                _szAttachmentName = Utilities.GetConfigValue("AUSAttachmentName", "AUS_Report_{0}_{1}.pdf");
                _szSubject = Utilities.GetConfigValue("AUSSubject", "Your Alerts Report is Enclosed");
                _szSubject_Spanish = Utilities.GetConfigValue("AUSSubject_Spanish", "Su informe de alertas está adjunto");
                _szCategory = Utilities.GetConfigValue("AUSCategory", string.Empty);
                _szSubcategory = Utilities.GetConfigValue("AUSSubcategory", string.Empty);
                _bDoNotRecordCommunication = Utilities.GetBoolConfigValue("AUSDoNotRecordCommunication", true);
                ...
                if (_iMonitoredCompanyID <= 0) {
                    ...
                    UpdateDateTimeCustomCaption(_oConn, "prau_LastRunDate", _dtEndDate);
                }
                ...
                if (Utilities.GetBoolConfigValue("AUSSendResultsDetails", true)) {
                ...
                if (Utilities.GetBoolConfigValue("AUSWriteResultsToEventLog", true)) {
                    _oBBSMonitorService.LogEvent(sbMsg.ToString());
                }

                if (Utilities.GetBoolConfigValue("AUSSendResultsToSupport", true)) {
                    SendMail("AUS Reports Success", sbMsg.ToString());
                }
            } catch (Exception e) {
                LogEventError("Error Procesing AUS Reports.", e);
                ...
                sbMsg.Append("\n\n" + e.Message);
                sbMsg.Append("\n\n" + e.StackTrace);
                SendMail("AUS Reports Error", sbMsg.ToString());
        ...
        public void GenerateFax()
        {
            ...
            oSQLCommand.Parameters.AddWithValue("@ReceiveMethod", "1"); //Fax
            ...
            foreach (ReportUser oReportUser in lReportUser)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (_iMonitoredCompanyID > 0)
                        {
                            abAUSReport = oReportInterface.GenerateAUSReport(oReportUser.PersonID, oReportUser.CompanyID, _dtStartDate, _dtEndDate, _iMonitoredCompanyID);
                        }
                        else
                        {
                            abAUSReport = oReportInterface.GenerateAUSReport(oReportUser.PersonID, oReportUser.CompanyID, _dtStartDate, _dtEndDate);
                        }

                        string _szContent;
                        string szSubjectCalculated;
                        if (oReportUser.Culture == "es-mx")
                        {
                            _szContent = string.Format(GetEmailTemplate("AUSContent_S.html"), oReportUser.Method.ToLower(), _dtNextDateTime.ToString("MMMM d, yyyy"), "");
                            szSubjectCalculated = _szSubject_Spanish;
                        }
                        else
                        {
                            _szContent = string.Format(GetEmailTemplate("AUSContent.html"), oReportUser.Method.ToLower(), _dtNextDateTime.ToString("MMMM d, yyyy"), "");
                            szSubjectCalculated = _szSubject;
                        }
                        ...
                        SendReport(_oConn,
                                   ...
                                   _szContent,
                                   abAUSReport,
                                   string.Format(_szAttachmentName, oReportUser.CompanyID, oReportUser.PersonID),
                                   _szCategory,
                                   _szSubcategory,
                ...
                catch (Exception e)
                {
                    ...
                    LogEventError("Error Generating AUS Report.", e);
        ...
        public void GenerateEmail()
        {
            ...
            oSQLCommand.Parameters.AddWithValue("@StartDate", _dtStartDate);
            oSQLCommand.Parameters.AddWithValue("@EndDate", _dtEndDate);
            oSQLCommand.Parameters.AddWithValue("@ReceiveMethod", "2"); //Email -- end of Day
            ...
            // Now go generate a report for each one of 
            // these and send it out
            ...
            foreach (ReportUser oReportUser in lReportUser)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        //Add images at top and bottom
                        _szContent = szEmailImageHTML_TOP + _szContent + szEmailImageHTML_BOTTOM;

                        _iEmailCount++;
                        _szContent = GetFormattedEmail(_oConn, _oTran, oReportUser.CompanyID, oReportUser.PersonID, szSubjectCalculated, _szContent);
