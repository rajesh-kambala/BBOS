...
function GetDetails(emailaddress)
{
  ...
  if (emailaddress != "")
  {
    ...
    {  
      ...
      {
        // get the lead info
        SQL = "SELECT Lead_LeadID FROM Lead WHERE UPPER(RTRIM(Lead_PersonEMail)) = UPPER(RTRIM(N'" +
          emailaddress + "')) AND Lead_Deleted IS NULL";
        recQuery = eWare.CreateQueryObj(SQL);
        MsgHandler.Log(SQL);
        recQuery.SelectSQL();
        if (recQuery.EOF)
...
function prepareConfirmationMail()
{
  ...
  // Add in the system notify address
  SQL = "SELECT Parm_Value FROM Custom_SysParams WHERE LOWER(RTRIM(Parm_Name)) = N'notifyemailaddress'";
  recQuery = eWare.CreateQueryObj(SQL);
  MsgHandler.Log(SQL);
  recQuery.SelectSQL();
  if (!recQuery.EOF)
  {
    eMail.SenderAddress = recQuery("Parm_Value");
  }
  if (eMail.SenderAddress == "")
  {
    MsgHandler.Log("Notify E-mail Address is not set.");
  }
    
  SQL = "SELECT Parm_Value FROM Custom_SysParams WHERE LOWER(RTRIM(Parm_Name)) = N'notifyemailname'";
  recQuery = eWare.CreateQueryObj(SQL);
  MsgHandler.Log(SQL);
  recQuery.SelectSQL();
  if (!recQuery.EOF)
  {
    eMail.SenderName = recQuery("Parm_Value");
  }
  if (eMail.SenderAddress == "")
  {
    MsgHandler.Log("Notify E-mail Name is not set.");
  }
}
