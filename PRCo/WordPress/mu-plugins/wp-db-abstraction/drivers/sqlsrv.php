...
<p>If you don\'t know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="http://wordpress.org/support/">WordPress Support Forums</a>.</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, $this->dbuser ), 'db_select_fail' );
     ...
     */
    function prepare2( $query = null ) { // ( $query, *$args )
        if ( is_null( $query ) )
            return;
    ...
    function prepare( $query, ...$args ) { // ( $query, *$args )
        if ( is_null( $query ) )
            return;
        $this->prepare_args = func_get_args();
        array_shift($this->prepare_args);
    ...
    function db_connect($allow_bail = true) {
		...
		$connection_info["Encrypt"] = "yes";
		$connection_info["TrustServerCertificate"] = true;
...
"/*/WP_I18N_DB_CONN_ERROR*/, $this->dbhost ), 'db_connect_fail' );
    ...
    function query( $query, $translate = true ) {
		...
		// CHW   http://wordpress.org/support/topic/fresh-wp-35-install-not-compatible?replies=5
    ...
    function _post_query($dbh, $query) {
        ...
        if ( preg_match("/^\\s*(insert|delete|update|replace) /i",$query) ) {
			...
			$this->affected_rows = $this->rows_affected;
    ...
    function log_query( $query, $query_time = null, $query_callstack = null, $query_start = null, $query_data = null )
    {
        if (!defined('SAVEQUERIES') || !SAVEQUERIES) {
            return; //bail
        }
